...
from .models import Category, Product, Review, FavoriteProducts, Mail, Customer, ShippingAddress
...
class ProductList(ListView):  # template_name isn't necessary, but we'll add
    model = Product
...
class CategoryView(ListView):
    ...
    def get_queryset(self):
        ...
        products = Product.objects.filter(category__in=subcategories)  # All products of all subcategories
...
class ProductDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for i in range(4):
            ...
            if p not in data and p != product:
                data.append(p)
...
class FavoriteProductsView(LoginRequiredMixin, ListView):  # LoginRequiredMixin - checks if user logged in
    # else - sent him back to login_url
    model = FavoriteProducts
    context_object_name = 'products'
    template_name = 'store/favorite_products.html'
...
def send_mail_to_all(request):
    ...
    if request.method == 'POST':
        ...
        for email in mail_list:
            mail = send_mail(
                subject='We have a new offer',  # title of mail
                message=text,
                from_email=settings.EMAIL_HOST_USER,
                recipient_list=[email],
                fail_silently=False  # ignore any errors while sending a mail
            )
            if mail:
                messages.success(request, f'The message has been sent successfully to the email {email}')
            else:
                messages.error(request, f'The message has NOT been sent to the email {email}')
    else:
        pass
    context = {
        'title': 'Mail customers'
    }
    return render(request, 'store/send_mail.html', context)
...
def to_cart(request, product_id, action):
    if request.user.is_authenticated:
        ...
        messages.error(request, 'SIGN IN/SIGN UP in order to shop')
        return redirect('login_registration')
